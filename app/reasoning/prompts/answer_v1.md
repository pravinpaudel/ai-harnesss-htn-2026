...
4. Answer with the entity's label and name (e.g. "TD (TD Bank)") and the quarter. Cite one handle per clue, preferring evidence marked `in_period` (from that quarter's own narrative) over summary tables, overviews or other quarters. Put the entity label in `values` as `{"label": "company", "value_text": "<label>", "unit": "text"}`.
